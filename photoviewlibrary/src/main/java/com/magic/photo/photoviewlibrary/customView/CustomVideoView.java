...
import com.magic.photo.photoviewlibrary.utils.AdParameters;
import com.magic.photo.photoviewlibrary.utils.SDKConstant;
...
public class CustomVideoView extends RelativeLayout implements View.OnClickListener,
    ...
    private VideoPlayerDurationListener durationListener;
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        if (mediaPlayer != null) {
            ...
            if (durationListener != null) {
                durationListener.onVideoDuration(getDuration());
            }
            //满足自动播放条件，则直接播放
            boolean canAutoPlay = Utils.canAutoPlay(getContext(), AdParameters.getCurrentSetting());
            if (canAutoPlay) {
    ...
    public void setDurationListener(VideoPlayerDurationListener listener) {
    ...
    private void showPauseView(boolean show) {
        ...
        if (!show) {
            if (isComplete()) {
                ...
                Glide.with(getContext()).load(Uri.fromFile(new File(mUrl))).into(mFrameView);
     ...
     * 增加个获取时长的回调
     */
    public interface VideoPlayerDurationListener {
